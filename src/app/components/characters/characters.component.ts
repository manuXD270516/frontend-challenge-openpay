...
export class CharactersComponent implements OnInit, AfterViewInit {
  ...
  get() {
    this.scroll$.pipe(
      ...
      concatMap(offset => {
        ...
        return this.marvelService.getData(this.category);
  ...
  search() {
    this.searchText$.pipe(
      debounceTime(400),
      distinctUntilChanged(),
      switchMap(() => this.marvelService.getData(this.category))).subscribe(data => this.handleResponse(data, true));
  }
  ...
  handleResponse(data: any, reset: boolean = false) {
    this.characters = reset ? data : [...this.characters, ...data];
